...
{   
    ...
    for (i = 0; i < 8; ++ i) {
        plaintext[i] = 0;
        for (j = 0; j < 6; ++ j) {
            shamt = 32 - plaintext_32bit_expanded_to_48bit_table[i * 6 + j];
            plaintext[i] = (plaintext[i] << 1) | !!((1 << shamt) & r);
        }
        plaintext[i] ^= subkey[i]; //XOR with key
    }
    //search sbox & combine into a new 32bit string (r here)
    r = 0;
    for (i = 0; i < 8; ++ i) {
        row = ((plaintext[i] >> 4) & 0x2) | (plaintext[i] & 0x1);
        col = (plaintext[i] >> 1) & 0xf;
        r = (r << 4) | sbox[i][(row << 4) + col];
    }
    rval = 0;
    for (i = 0; i < 32; ++ i) {
        shamt = 32 - sbox_perm_table[i];
        rval = (rval << 1) | !!((1 << shamt) & r);
    }
    return rval;
    //////////
