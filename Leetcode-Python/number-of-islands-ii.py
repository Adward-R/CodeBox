...
class Solution(object):
    def numIslands2_TLE(self, m, n, positions):
        """
        :type m: int
        :type n: int
    ...
    def numIslands2(self, m, n, positions):  # Union Find Version
        """
        :type m: int
        :type n: int
        :type positions: List[List[int]]
        ...
        def find_set(p):  # with path compression
            i, j = p
            path = []
        ...
        def union(p1, p2):  # p1 and p2 are tuples
            r1, r2 = find_set(p1), find_set(p2)
            if r1 != r2:
                father[r1[0]][r1[1]] = r2
            return r1 != r2
        ...
        count = 0
        for x, y in positions:
            next_coors = ((x-1, y), (x+1, y), (x, y-1), (x, y+1))
            nbrs = list(filter(lambda p: 0 <= p[0] < m and 0 <= p[1] < n and father[p[0]][p[1]] is not None, next_coors))
            if len(nbrs) == 0:
                father[x][y] = (-1, -1)
                count += 1
            elif len(nbrs) == 1:
                ...
                ind = randint(0, len(nbrs)-1)
                father[x][y] = nbrs[ind]
                for i in range(len(nbrs)):
                    if i != ind:
                        count -= union(nbrs[i], nbrs[ind])
            ans.append(count)
        return ans
...
ans = sol.numIslands2(21, 71, positions)
print(ans == expected)