...
class Solution(object):
    ...
    def majorityElement1(self, nums):  # correct method
        """
        :type nums: List[int]
        :rtype: List[int]
        """
        ...
        for i in range(len(nums)):
            if nums[i] in freqElmts:
                freqElmts[nums[i]][0] += 1
                freqElmts[nums[i]][1] = i
            else:
                if len(freqElmts) >= 4:
                    oldest = min(freqElmts, key=lambda x: i if freqElmts[x][0] > 1 else freqElmts[x][1])
                    del freqElmts[oldest]
                freqElmts[nums[i]] = [1, i]

        # print(freqElmts)
        for key in freqElmts:
            freqElmts[key] = 0
...
nums = [1,2,1,2,4,2,1,2,5,1,6,7,1,2]
