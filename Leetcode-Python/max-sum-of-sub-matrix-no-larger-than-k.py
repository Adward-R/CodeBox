...
class Solution(object):  # 2D Kadane Algorithm
    def maxSumSubMatrix(self, matrix, k):
        ...
        for col in range(n):
            ...
            for r in range(col, n):
                ...
                # find the max sub-array <= K
                accu = [0]  # not monotonically ascending!
                curSum, curMax = 0, - 2 << 31
                for s in sums:
                    curSum += s
                    ind = bisect_left(accu, curSum - k)
                    if ind < len(accu):
                        curMax = max(curMax, curSum - accu[ind])
                    insort(accu, curSum)  # accu.append(curSum)
                ans = max(ans, curMax)
        return ans
...
# print(sol.maxSumSubMatrix(matrix, 8))
...
print(sol.maxSumSubMatrix(matrix, -100))